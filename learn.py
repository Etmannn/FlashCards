    ...
    while asking == True:
        ...
        for key in range(rangein - 1, rangeout):
            if combined_dict.get(key) == 0:
                correct = 0
                correct = common_functions.mutlichoice(file=file, question=key)
                if correct > 0:
                    multichoice[key] += 1
            else:
                correct = 0
                correct = common_functions.write(file=file, question=key)
                if correct > 0:
                    write[key] += 1
        ...
        if total >= totalscore:
            asking = False
            input("Press enter to move onto next segment: ")
...
if qn < 4:
    ...
    if group == 0:
        ...
        for rangein, rangeout in zip(rangeinlist, rangeoutlist, strict=True):
            learn(qn=qn, rangein=rangein, rangeout=rangeout)
